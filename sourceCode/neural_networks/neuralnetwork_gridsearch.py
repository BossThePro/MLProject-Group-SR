...
def progress_ui(epoch_num, epoch_max, loss_val, loss_train, r_sq, last_epoch_time, average_epoch_time, best_loss):
...
param_grid = {
    ...
    'num_epochs': [30],
    'hidden_layer_1': [192],
    'hidden_layer_2': [192, 48],
    'hidden_layer_3': [192,96,34],
    'hidden_activation': ["ReLU", "ELU"],
    'output_activation': ["Softplus", "Exponential"]
}
...
def evaluate_params(params):
    ...
    # training parameters
    n_epochs = params['num_epochs']  # number of epochs to run
    ...
    for epoch in range(n_epochs):
        ...
        with tqdm.tqdm(batch_start, unit="batch", mininterval=0, disable=True) as bar:
            ...
            # start = np.random.choice([x for x in bar])
                ...
                if params['output_activation'] == "Exponential":
                    y_pred = torch.exp(y_pred) # * torch.tensor(exposure_batch)
                elif params['output_activation'] == "Softplus":
        ...
        progress_ui(epoch_num=epoch, epoch_max=n_epochs, loss_val=loss_val, loss_train=loss_train, r_sq=r_sq, last_epoch_time=last_epoch_time, average_epoch_time=average_epoch_time, best_loss=best_loss)
    ...
    return {
        'params': params,
        'loss': best_loss,
        'var': float(torch.var(y_predicted)),
        'max': float(torch.max(y_predicted)),
        'min': float(torch.min(y_predicted)),
        'r2': float(r2_score(y_val.numpy(), y_predicted.detach().numpy()))
    }
